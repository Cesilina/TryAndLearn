...
def scaled_dot_product_gqa(
    ...
    if num_head_groups > 1 or force_grouped:
        ...
        res = torch.matmul(query, key.transpose(-1, -2))
...
if __name__ == '__main__':
    ...
    gqa = MultiheadGQA(embed_dim=embed_dim,query_heads=query_heads, kv_heads=kv_heads)

    query = torch.randn((10,16,64))
    key = torch.randn((10, 16, 64))
    value = torch.randn((10,16,64))
    x, attn = gqa(query, key, value)
    print(x.shape)