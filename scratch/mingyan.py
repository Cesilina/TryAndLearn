...
def getDetailUrl(filePath,arrUrl):
    ...
    while 1:
        ...
        for index,articleUrl in enumerate(detailList):
            try:
                f=open('%s/%s.txt'%(filePath,detailName[index]),'a')
                print(articleUrl)
                response=requests.get(url+articleUrl)
                response.encoding = 'gb2312'
                selector = html.fromstring(response.text)
                P_element = selector.xpath('//*[@id="p_left"]/div[1]/div[4]/div//div') # span
                for p in P_element:
                    if len(p.text)==1:
                        continue
                    else:
...
url='http://www.geyan.net/'
response=requests.get(url)
response.encoding = 'gb2312'
selector=html.fromstring(response.text)
myArr=selector.xpath('//*[@id="p_left"]/div/dl/dt/strong/a/@href') # 获取strong下所有a的元素属性href的所有元素
nameArr=selector.xpath('//*[@id="p_left"]/div/dl/dt/strong/a/text()') # 获取strong下所有a的元素的文本信息
print(len(myArr))

for index, detail in enumerate(myArr):
    filePath=os.path.join(root_path , 'data' + '/%s'%(nameArr[index]))
    createFile(filePath)
    arrUrl=url+detail
    print(arrUrl)
