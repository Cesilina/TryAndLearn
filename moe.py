...
class MoE(nn.Module):
    ...
    def forward(self, x):
        # 获取批量大小 x:[batch_size, seq_len]-->input_ids
        batch_size = x.size(0)
...
for epoch in range(num_epochs):
    ...
    for features, labels in train_loader:
        ...
        for name in model.named_parameters():
            if "experts" in name[0] and "weight" in name[0]:
                print(name[1].grad)
        optimizer.step()
        total_loss += loss.item()
        eval_acc = evaluate(model, test_loader)
    print(f'Epoch {epoch+1}, Loss: {total_loss/len(train_loader)}, eval_acc:{eval_acc}')
